...
fit_feature_lmm <- function(data_long, feature, group_col = "Group", 
  ...
  # Ensure strict typing for the model
  df_model <- data.frame(
  ...
  # Drop NAs for this specific feature to prevent model crash
  ...
  tryCatch({
    # Formula: Value ~ Time + Group + Time:Group + (1 | ID)
    formula_str <- "Value ~ Time * Group + (1 | ID)"
    ...
    # Identify the interaction term (e.g., TimeT1:GroupResponder)
    ...
    if (length(interaction_idx) == 1) {
      ...
      # Ensure Pr(>|t|) exists (lmerTest provides this)
    ...
    # Silent fail for un-computable features (Singular fit, 0 variance, etc.)
  }, warning = function(w) {
    # Singular fit warnings are acceptable, model still outputs estimates
  })
...
#' @description Plots the Estimate vs -log10(FDR) of the interaction terms.
...
plot_lmm_volcano <- function(results_df, title = "Longitudinal Interaction (LMM)") {
  ...
  plot_df <- results_df %>% filter(!is.na(P_Value_Interaction))
  ...
  plot_df$Significance[plot_df$FDR_Interaction < 0.05 & plot_df$Estimate_Interaction > 0] <- "Positive Interaction"
  plot_df$Significance[plot_df$FDR_Interaction < 0.05 & plot_df$Estimate_Interaction < 0] <- "Negative Interaction"
  
  # Optional: Fallback to raw P-value labeling if nothing survives FDR (for visual exploration)
  if (all(plot_df$Significance == "Not Significant")) {
    plot_df$Significance[plot_df$P_Value_Interaction < 0.05 & plot_df$Estimate_Interaction > 0] <- "Positive (Raw P < 0.05)"
    plot_df$Significance[plot_df$P_Value_Interaction < 0.05 & plot_df$Estimate_Interaction < 0] <- "Negative (Raw P < 0.05)"
  }
  ...
  p <- ggplot(plot_df, aes(x = Estimate_Interaction, y = logFDR, color = Significance)) +
    ...
    labs(
      title = title,
      subtitle = "X: Estimate of Time:Group Interaction | Y: -log10(FDR)",
      x = "LMM Interaction Estimate",
      y = "-log10(FDR)"
    ) +
